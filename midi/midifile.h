...
// All 256 possible midi channels aren't needed on floppy drives, so 8 should be enough.
// Polyphony got dropped at all since every event gets read one by one from sd card.
// This settings will only need ~400 Byte instead of ~130KB
//
// - coon

#define MAX_MIDI_TRACKS		 11 // default: 256 (8 + 1 invisible track)
					...
					union {
						...
						struct {
								...
								union {
									...
									struct {
										//BYTE			*pData;
										BYTE pData[64]; // 128 byte for meta events texts... (hope this is enough)
